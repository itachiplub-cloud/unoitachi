...
from database import get_group_config, set_group_config  # Youâ€™ll need to define these
...
async def set_welcome(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if member.status not in ["administrator", "creator"]:
        await update.message.reply_text("ðŸš« Only admins can set the welcome message.")
        return
...
async def set_farewell(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    member = await context.bot.get_chat_member(update.effective_chat.id, update.effective_user.id)
    if member.status not in ["administrator", "creator"]:
        await update.message.reply_text("ðŸš« Only admins can set the farewell message.")
        return

    text = " ".join(context.args)
    if not text:
        await update.message.reply_text("Usage: /setfarewell Goodbye {name}, weâ€™ll miss you!")
        return

    set_group_config(update.effective_chat.id, "farewell_msg", text)
    await update.message.reply_text("âœ… Farewell message set.")
...
async def member_update_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if status_change == "joined":
        ...
        msg_template = get_group_config(chat_id, "welcome_msg") or "ðŸ‘‹ Welcome {name}!"
        ...
        msg_template = get_group_config(chat_id, "farewell_msg") or "ðŸ‘» Goodbye {name}!"
        msg = msg_template.format(name=user.full_name, username=user.username or "")
        await context.bot.send_message(chat_id=chat_id, text=msg)
