        ...
        try:
            ...
            with db_lock:
                ...
                with get_conn() as conn:
                    ...
                    print(f"Ã¢Å“â€¦ Updated balance for UID {uid} by Ã¢â€šÂ¹{amount}")

                    return True

        except sqlite3.OperationalError:

            print(f"Ã¢Å¡Â Ã¯Â¸Â Attempt {attempt+1}: DB locked for UID {uid}, retrying...")

            time.sleep(1)

    print(f"Ã¢ÂÅ’ Failed to update balance for UID {uid}")
    ...
    for attempt in range(3):
        ...
        try:
            ...
            with db_lock:
                ...
                print(f"Ã¢Å“â€¦ Bank cleared for UID {uid}")

                return True

        except sqlite3.OperationalError:

            print(f"Ã¢Å¡Â Ã¯Â¸Â Attempt {attempt+1}: DB locked during set_bank, retrying...")

            time.sleep(1)

    print(f"Ã¢ÂÅ’ Failed to clear bank for UID {uid}")
        ...
        try:
            ...
            with db_lock:
                ...
                with get_conn() as conn:
                    ...
                    print(f"Ã¢Å“â€¦ Deposited Ã¢â€šÂ¹{amount} into tax bank")

                    return True

        except sqlite3.OperationalError:

            print(f"Ã¢Å¡Â Ã¯Â¸Â Attempt {attempt+1}: DB locked during tax deposit, retrying...")

            time.sleep(1)

    print(f"Ã¢ÂÅ’ Failed to deposit tax after retries")
...
def migrate_users_table():
    ...
    with get_conn() as conn:
        ...
        try:
            ...
            print("Ã¢Å“â€¦ chat_id column added to users table.")

        except sqlite3.OperationalError as e:

            if "duplicate column name" in str(e):

                print("Ã¢â€žÂ¹Ã¯Â¸Â chat_id column already exists.")
...
def backfill_chat_ids(default_chat_id):
    ...
    with get_conn() as conn:
        ...
        print(f"Ã°Å¸Â§Â¹ Backfilled chat_id for legacy users with {default_chat_id}")
...
def force_patch_all_users(chat_id):
    ...
    with get_conn() as conn:
        ...
        print(f"Ã°Å¸â€ºÂ Ã¯Â¸Â Force patched all users with chat_id {chat_id}")
...
def debug_users_by_chat(chat_id):
    ...
    with get_conn() as conn:
        ...
        print(f"Ã°Å¸â€œÅ  Found {len(rows)} users with chat_id {chat_id}")
...
def get_conn():
    ...
    ContextÃ¢â‚¬Âmanager for SQLite connections.
...
def initialize_database():
    ...
    Call this on bot start to ensure your schema is upÃ¢â‚¬ÂtoÃ¢â‚¬Âdate.
...
if __name__ == "__main__":
    ...
    print(f"Ã¢Å“â€¦ Clan database initialised at '{DB_PATH}'")
