...
def migrate_giveaway_card_table():
    with get_conn() as conn:
        ...
        if "image_file_id" not in column_names:
            ...
            print("âœ… image_file_id column added.")
...
async def uploadgiveawaycard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not replied or not replied.photo:
        return await update.message.reply_text(
            "âš ï¸ Reply to a photo and use caption:\n<name> | <rarity> | <value> | <description>"
        )
    ...
    try:
        ...
        value = int(value)
        description += " â€” SPECIAL EDITION (Not available from draw)"
        image_file_id = replied.photo[-1].file_id

        add_giveaway_card(name, rarity, value, description, image_file_id)
        await update.message.reply_text(
            f"âœ… <b>{name}</b> added as a SPECIAL EDITION card with value <b>{value}</b>.",
            parse_mode="HTML"
        )
    except Exception as e:
        await update.message.reply_text(
            f"âš ï¸ Failed to upload card. Make sure your caption is formatted like:\n<name> | <rarity> | <value> | <description>\n\nError: {e}"
        )
...
async def editgiveawaycardbyindex(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with get_conn() as conn:
            ...
            conn.commit()
        await update.message.reply_text(f"âœ… Card ID {card_id} updated.")
    except:
        await update.message.reply_text(
...
async def givecard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"âœ… Card ID {card_id} given to user {target_uid}.")
    except:
...
async def mygiveaways(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        return await update.message.reply_text("ðŸŽ You donâ€™t have any giveaway cards yet.")

    for name, rarity, value, desc, image_file_id in rows:
        caption = f"<b>{name}</b> ({rarity}) â€” ðŸ’° {value} coins\n{desc}"
...
async def giveawaylist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_conn() as conn:
        ...
        """).fetchall()
    if not rows:
        return await update.message.reply_text("ðŸ“­ No cards assigned yet.")
    lines = ["ðŸ“‹ <b>Assigned Giveaway Cards</b>"]
    for uid, name, rarity, value in rows:
        lines.append(f"â€¢ <b>{name}</b> ({rarity}) â€” ðŸ’° {value} â†’ <a href='tg://user?id={uid}'>User</a>")
...
async def removegivecard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"âŒ Card ID {card_id} removed from user {target_uid}.")
    except:
...
async def giveawaycards(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cards:
        return await update.message.reply_text("ðŸ“­ No giveaway cards available.")

    lines = ["ðŸŽ <b>Available Giveaway Cards</b>"]
    for i, (card_id, name, rarity, value, desc) in enumerate(cards, 1):
        lines.append(f"{i}. <b>{name}</b> ({rarity}) â€” ðŸ’° {value} coins\n{desc}")
...
async def deletegiveawaycard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if index < 1 or index > len(cards):
            return await update.message.reply_text("âš ï¸ Invalid card index.")

        card_id = cards[index - 1][0]
        delete_giveaway_card(card_id)
        await update.message.reply_text(f"ðŸ—‘ï¸ Card ID {card_id} deleted from system.")
    except Exception as e:
        await update.message.reply_text(f"âš ï¸ Usage:\n/deletegiveawaycard <card_index>\nError: {e}")
