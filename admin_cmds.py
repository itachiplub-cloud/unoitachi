...
async def resetwallets(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db_lock:
        with get_conn() as conn:
            ...
            conn.commit()
    await update.message.reply_text("ðŸ§¹ All user coin balances reset to â‚¹0.")
...
async def resetdeposit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db_lock:
        with get_conn() as conn:
            ...
            conn.commit()
    await update.message.reply_text("ðŸ¦ All user deposits have been reset.")
...
async def resetbank(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db_lock:
        with get_conn() as conn:
            ...
            conn.commit()
    await update.message.reply_text("ðŸ›ï¸ Central bank reserve wiped.")
...
async def resetinvestments(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db_lock:
        with get_conn() as conn:
            ...
            conn.commit()
    await update.message.reply_text("ðŸ“‰ All investment (locked savings) wiped.")
...
async def resetassets(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("ðŸ§¹ All user assets (from /myassets) have been reset.")
...
async def resettea(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("ðŸ§¹ All /tea data has been reset.")
...
async def broadcastgroups(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg:
        return await update.message.reply_text("ðŸ“ Usage: /broadcastgroups Your message here")
    ...
    await update.message.reply_text(f"ðŸ“¡ Sent to {sent} group(s).")
...
async def broadcastdms(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = " ".join(context.args)
    if not msg:
        return await update.message.reply_text("ðŸ“ Usage: /broadcastdms Your message here")
    ...
    await update.message.reply_text(f"ðŸ“© Sent to {sent} user(s) in DM.")


dm_links = {}  # uid â†’ admin_id
...
async def dmchat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        return await update.message.reply_text("ðŸ“ Usage: /dmchat <user_id> <your message>")

    try:
        uid = int(context.args[0])
        msg = " ".join(context.args[1:])
        await context.bot.send_message(chat_id=uid, text=f"ðŸ“¬ Message for you:\n{msg}")
        dm_links[uid] = sender  # Store who initiated the chat
        await update.message.reply_text(f"âœ… Sent message to UID {uid}.")
    except Exception as e:
        await update.message.reply_text(f"âŒ Failed to send DM: {e}")
...
async def dm_reply_listener(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            chat_id=admin_id,
            text=f"ðŸ“¨ Reply from UID {uid}:\n{reply_text}"
        )
    except Exception as e:
        print(f"âŒ Failed to forward reply: {e}")
...
async def finduid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        return await update.message.reply_text("ðŸ“ Usage: /finduid @username")

    username = context.args[0]
    uid = get_user_by_username(username)

    if uid:
        await update.message.reply_text(f"ðŸ” UID of {username} is `{uid}`")
    else:
        await update.message.reply_text(f"âŒ Username {username} not found in database.")
...
async def wanted(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not raid_log:
        return await update.message.reply_text("ðŸš“ No wanted posters yet. Streets are quiet.")

    msg = "ðŸš¨ <b>Recent Police Raids:</b>\n"
    for i, entry in enumerate(raid_log[-10:], start=1):  # Last 10 raids
        uid = entry["uid"]
        loss = entry["loss"]
        time = entry["timestamp"]
        username = get_username(uid) or "Unknown"
        msg += f"{i}. @{username} â€” â‚¹{loss} stolen at {time}\n"
...
async def raid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    sender = update.effective_user.id
    if sender not in ADMIN_IDS:
        return await update.message.reply_text("ðŸš« Youâ€™re not authorized to use this command.")

    if context.args and context.args[0].lower() in ["on", "off"]:
        mode = context.args[0].lower()
        set_global_raid(mode == "on")
        status = "activated" if mode == "on" else "deactivated"
        emoji = "ðŸš¨" if mode == "on" else "ðŸ§¹"
    ...
    if context.args and context.args[0].startswith("@"):
        ...
        await context.bot.send_message(
            chat_id=uid,
            text="ðŸš¨ Youâ€™ve been manually raided by Reverse God Admins.\nðŸ’¸ â‚¹500 fine + your wallet emptied.\nðŸ•¶ï¸ Stay low â€” theyâ€™re watching you."
        )

        return await update.message.reply_text(f"âœ… Manual raid triggered on @{username}. â‚¹{total_loss} confiscated.")

    return await update.message.reply_text(
        "âš ï¸ Usage:\n"
        "/raid on â€” activate global raids (100% chance)\n"
        "/raid off â€” deactivate raids\n"
        "/raid @username â€” manual bust\n"
    )
...
async def unraid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("ðŸ§¹ Global raid mode manually cleared. Streets back to stealth.")
...
async def cleartax(update: Update, context: ContextTypes.DEFAULT_TYPE):
    sender = update.effective_user.id
    if sender not in ADMIN_IDS:
        return await update.message.reply_text("ðŸš« Youâ€™re not authorized to clear tax pool.")
    ...
    return await update.message.reply_text(f"ðŸ§¹ Tax pool cleared.\nðŸ’° â‚¹{pool} removed from circulation.")
